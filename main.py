...
async def start(ctx):
    if ctx.message.author == bot.owner:
        ...
        supportUses = "'+support' uses this week: " + str(c['supportUses'])
        await bot.send_message(bot.owner, pollUses)
        await bot.send_message(bot.owner, supportUses)
        c.clear()
...
async def srvrs(ctx):
    ...
    await bot.send_message(bot.owner, 'type +!more for deeper analytics')
    r = requests.post("https://bots.discord.pw/api/bots/{}/stats".format(bot.user.id), json={"server_count":len(bot.servers)}, headers={"Authorization":config.pwtoken})
    print('wow it actually worked lmao')
